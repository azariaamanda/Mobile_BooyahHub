...
enum OwnerNotificationType { info, warning, success, error }
...
class _OwnerNotificationPageState extends State<OwnerNotificationPage> {
  List<OwnerNotification> notifications = [];
  bool isLoading = true;
  ...
  Future<void> _fetchNotifications() async {
    try {
      ...
      final List<OwnerNotification> fetchedNotifications = [];
      for (var item in response) {
        final totalUtang = item['total_utang'] ?? 0;
        final limitUtang = item['limit_utang'] ?? 0;
        
        // Cek apakah admin terkena suspend (utang melebihi limit)
        if (totalUtang >= limitUtang && limitUtang > 0) {
          ...
          fetchedNotifications.add(
            OwnerNotification(
              id: notifId,
              title: 'Admin Terkena Suspend',
              message: 'Akun admin $adminName telah otomatis di-suspend karena utangnya (Rp$totalUtang) melebihi batas limit (Rp$limitUtang).',
              type: OwnerNotificationType.error,
  ...
  List<OwnerNotification> get filteredNotifications {
  ...
  Color _getTypeColor(OwnerNotificationType type) {
    switch (type) {
      case OwnerNotificationType.info:
        return AppColors.info;
      case OwnerNotificationType.warning:
        return AppColors.warning;
      case OwnerNotificationType.success:
        return AppColors.accent;
      case OwnerNotificationType.error:
        return AppColors.error;
    }
  }

  IconData _getTypeIcon(OwnerNotificationType type) {
    switch (type) {
      case OwnerNotificationType.info:
  ...
  Future<void> _markAsRead(String id) async {
    setState(() {
      ...
      if (index != -1) {
        notifications[index] = OwnerNotification(
  ...
  void _deleteOwnerNotification(String id) {
    setState(() {
  ...
  void _showNotificationDetails(OwnerNotification notification) {
  ...
  Widget _buildNotificationCard(OwnerNotification notification) {
              ...
              PopupMenuButton(
                ...
                itemBuilder: (context) => [
                  ...
                  PopupMenuItem(
                    ...
                    onTap: () {
                      _deleteOwnerNotification(notification.id);
                    },
                  ),
                ],
                icon: const Icon(
