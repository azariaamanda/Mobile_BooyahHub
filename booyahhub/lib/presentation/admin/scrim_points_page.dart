...
class _ScrimPointsPageState extends State<ScrimPointsPage> {
  ...
  Future<void> _fetchTeamsData(int sesiId) async {
    try {
      ...
      for (var p in pendaftaran) {
        ...
        Map<String, dynamic>? existingHasil;
        for (var h in hasil) {
          if (h['id_pendaftaran'] == p['id_pendaftaran']) {
            existingHasil = h;
            break;
          }
        }

        final kill = existingHasil != null ? (existingHasil['total_kill'] ?? 0) : 0;
        final place = existingHasil != null ? existingHasil['placement'] : null;

        int poinPlacement = 0;
        if (place != null) {
          for (var ps in _poinSystem) {
            if (ps['placement'] == place) {
              poinPlacement = ps['poin_placement'] ?? 0;
              break;
            }
          }
        }
  ...
  void _updatePoin(int index) {
    ...
    if (_teams[index]['place'] != null) {
      for (var ps in _poinSystem) {
        if (ps['placement'] == _teams[index]['place']) {
          poinPlacement = ps['poin_placement'] ?? 0;
          break;
        }
      }
